...
* 安装依赖：npm i
* 本地运行：npm run dev
* 打包后显示文件大小及分布：npm run build --report
* 打包到cordova：npm run buildApp
* 翻译语言包：npm run translate
...
* [tab缩进2Size](#tab缩进2Size)
* [使用jsDoc对函数进行注释](#使用jsDoc对函数进行注释)
* [使用cssrem对px单位进行转换实现适配](#使用cssrem对px单位进行转换实现适配)
